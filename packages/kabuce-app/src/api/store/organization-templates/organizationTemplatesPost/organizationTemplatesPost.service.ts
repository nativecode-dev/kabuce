  ...
  constructor(
    ...
    this.form = new FormGroup({
      ...
      body: new FormGroup({
        ...
        mediatype: new FormControl('text/html', [Validators.pattern(/(application|audio|font|example|image|message|model|multipart|text|video|x-(?:[0-9A-Za-z!#$%&'*+.^_`|~-]+))\/([0-9A-Za-z!#$%&'*+.^_`|~-]+)/), Validators.required]),
        name: new FormControl(undefined, [Validators.maxLength(256), Validators.minLength(8), Validators.required]),
        organization_id: new FormControl(undefined, [Validators.pattern(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), Validators.required]),
