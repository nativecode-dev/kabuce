import { BodyParams, Controller, Get, Inject, PathParams, Post, Put, Returns, ReturnsArray } from '@tsed/common'
...
export class OrganizationTemplates {
  ...
  async all(@PathParams('organization_id') organization_id: string) {
    return this.templates.find({ selector: { organization_id } })
  }

  @Post()
  @Returns(201, Template)
  @Returns(409, { description: 'Conflict' })
  async post(@PathParams('organization_id') organization_id: string, @BodyParams(Template) template: Template) {
    const existing = await this.templates.findOne({
      selector: { _id: template.slug, organization_id },
    })

    if (existing) {
      return this.templates.upsert(template)
    }
