import { Controller, Get, Post, BodyParams, Put, ReturnsArray, PathParams, Inject, Returns } from '@tsed/common'
...
export class OrganizationAccounts {
  ...
  @Get('/:account_id')
  @Returns(404, { description: 'Not found' })
  get(@PathParams('organization_id') organization_id: string, @PathParams('account_id') account_id: string): Account {
  ...
  post(@PathParams('organization_id') organization_id: string, @BodyParams() account: Account): ModelKey<Account> {
  ...
  put(@PathParams('organization_id') organization_id: string, @BodyParams() account: Account): ModelKey<Account> {
    return { id: account._id, model: account }
  }
}
